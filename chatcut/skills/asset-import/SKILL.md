...
4. Verify with `read_project` `view:"assets"`: the asset should appear in the project with transcription starting. Wait for transcription via `track_progress` before transcript work, exactly as with other import paths. Check upload/local-only state separately before export, `pull_asset`, or remote frame decode.
