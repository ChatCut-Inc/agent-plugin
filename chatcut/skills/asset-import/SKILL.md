...
Transcription audio is prepared, uploaded, and started before the large asset byte upload whenever readable audio exists. If audio/video assets were imported, call `track_progress` with `target:"transcription"` before transcript/caption work. Wait for `target:"upload"` only before byte-dependent work such as export/render, frame inspection, `pull_asset`, or anything requiring `remoteUrl`/media bytes.
...
If the helper reports a missing or broken media binary, first attempt to fix it yourself using available shell/package-manager capabilities in the current environment. Only ask the user to install or fix ffmpeg if self-repair is unavailable, blocked, or fails. Do not guess or rewrite helper metadata.
