  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0 || options.allowFailure) resolve({ code, stdout, stderr });
        ...
        reject(
          new Error(
            `${bin} failed with exit code ${code}${stderr ? `: ${stderr.slice(0, 2000)}` : ""}`,
          ),
        );
    });
  });
...
async function prepareTranscriptionAudio(input, metadata, assetType, context) {
  ...
  const outputPath = join(
    context.workDir,
    `chatcut-${context.nameNoExt}-transcription-${process.pid}-${context.index}.ogg`,
  );
  await run(
    context.options.ffmpeg,
    [
  ...
  if (!(await transcriptionAudioHasSignal(outputPath, context.options))) {
    logProgress(
  ...
  return {
    ...
    transcriptionAudio: {
      contentType: "audio/ogg",
      filename: replaceExtension(context.filename, "-transcription.ogg"),
      path: outputPath,
      size: await fileSize(outputPath),
    },
  };
...
async function runSingleImport(index, total, sourceFile, options) {
  ...
  const partialItem = {
    ...
    agentNext:
      "Asset is registered and can be edited now; upload is still pending. Wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes.",
  };
  ...
  if (options.background) {
    ...
    return {
      ...partialItem,
      agentNext:
        "Asset is registered and can be edited now; do not wait for upload just to continue timeline work. For most editing tasks, wait only for target=transcription before transcript/caption/script-aware work. Wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes. If background statusFile reports an error, use retry.assetUploadArgs or retry.transcriptionOnlyArgs with a fresh import token.",
...
async function runSessionImport(options) {
  ...
  assertTool(options.ffmpeg, "ffmpeg");
  assertTool(options.ffprobe, "ffprobe");
  if (!options.workDir)
  ...
  if (deferredUploads.length > 0) {
    const partialResult = {
      ...
      agentNext:
        "All assets are registered and you can start editing now. Uploads are still underway; wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes.",
    ...
    await emitJson(
      {
        ...
        mode: "session-imported",
        agentNext:
          "Registered assetIds can be edited immediately. For most editing tasks, wait only for target=transcription before transcript/caption/script-aware work. Wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes.",
        count: options.inputs.length,
        imports: finalItems,
      },
      options,
  ...
  await emitJson(
    {
      ...
      agentNext: options.background
        ? "All assets are registered and you can start editing now. Uploads are still underway in the background; wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes."
        : "Registered assetIds can be edited immediately. For most editing tasks, wait only for target=transcription before transcript/caption/script-aware work. Wait for target=upload only before byte-dependent work such as export/render, frame inspection, pull_asset, or anything requiring remoteUrl/media bytes.",
      count: options.inputs.length,
      ...(options.background
        ? {
...
async function main() {
  ...
  if (options.backgroundWorker) await runBackgroundWorker(options);
