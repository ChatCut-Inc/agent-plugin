...
Export runs through the connector: use the `export` skill with `submit_export`, then `track_export`. In Claude Code, follow that skill's Claude Code delivery section — download the finished render locally and present the in-conversation preview card as the default deliverable.
...
When structured input would reduce friction, load the `widget-forms` skill and follow its Claude Code recipe: call `ask_followup_questions` for the normalized card data, then render natively (inline HTML cards, the host structured-question tool, or plain chat) instead of sending a long multi-question paragraph. Do not include media upload as a form question; ask for missing source media separately through a supported import path (see `asset-import`). Do not emit raw internal ChatCut chat tags directly to the user.
...
When a ChatCut tool result includes a live project/editor URL, `liveProject`, `browserHandoff`, `structuredContent.browserHandoff.required=true`, `browserHandoff.required=true`, or an internal-browser open-mode id such as `liveProject.openStrategy.preferredMode: "codex-internal-browser"`, treat it as a host in-app-browser instruction and open the URL in the Claude Code desktop Browser pane (`Claude_Browser` MCP tools: `preview_start {url}`, `navigate`, `screenshot`). Use `browserHandoff.url` when present; otherwise use the returned `editorUrl`. Preserve all query parameters, especially `dockviewLayout=media` and `editor-boot-token`, but REMOVE `theme=codex`: that parameter switches the editor into the Codex-webview mode that blocks manual file drops and hides import buttons, which does not apply to the Browser pane — the user should keep full manual control there.
...
- Keep ONE editor tab and reuse it for the whole session (`preview_start {url}` reuses the main tab; avoid `tabs_create` — a fresh tab re-triggers origin approval).
...
For any direct user-facing Markdown link or external-browser link, use the clean `editorUrl` and do not include host-only `dockviewLayout` or `editor-boot-token` query parameters. If you only have an internal-browser URL, strip those two parameters before showing it to the user.
