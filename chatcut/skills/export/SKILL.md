...
## Claude Code delivery: local file + in-conversation preview (verified 2026-07-12)

In Claude Code, do not stop at pasting the `downloadUrl`. After `track_export` returns the finished render:

1. DOWNLOAD locally: `curl -L -o ~/Downloads/<name>.mp4 "<downloadUrl>"` (tell the user the exact path; ask first only if they have expressed download preferences).
2. PREVIEW in the conversation — this is the DEFAULT deliverable, not optional. "给我看预览"/"show me a preview" means THIS card, not the browser pane. The widget tool (`show_widget` on the `visualize` server) is a deferred builtin in Claude Code desktop: if it is not in your visible tool list, discover it with `ToolSearch` ("show_widget" / "visualize") and load its `read_me` first — do not conclude it is absent without searching. Extract a poster frame from the local file (`ffmpeg -ss <mid> -i <file> -frames:v 1 -q:v 4 poster.jpg`, downscale to <=480px, embed as a `data:image/jpeg;base64,` URI — widget CSP blocks non-CDN network loads, so never point `<img>`/`<video>` at the S3 URL) and render a compact export card: poster, name, duration, resolution, file size, plus two `sendPrompt` buttons — "在浏览器面板播放" and "继续调整这条时间线". Keep the card under ~150 KB. Only if ToolSearch genuinely finds no widget tool (e.g. terminal CLI), report the local path and offer the pane instead.
3. PLAY in the pane — OPT-IN ONLY. Never navigate the Browser pane to the `downloadUrl` unless the user explicitly asks to play it there (e.g. clicks the card's play button or says so) AFTER the card is shown. Before navigating, tell the user the render bucket is a new origin so the pane will show a one-click approval card — an unannounced "navigation denied" reads as a bug. After they approve, retry the same navigation once. The user can also just open the downloaded local file directly.

Codex hosts keep the existing behavior: return the `downloadUrl` link.
