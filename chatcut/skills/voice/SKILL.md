...
- For long narration, split at sentence boundaries and generate short segments.
...
**Note**: For long text, split at sentence boundaries.
Do not use generic names like `part1` or `VO: part 1`; let the tool auto-name
from the actual text unless a truly descriptive custom name is needed.

```ts
// Long text - split into segments, keep text-based names
mcp__skill__submit_voice({
  provider: "elevenlabs",
  text: "First sentence.",
  voiceId: "peter",
