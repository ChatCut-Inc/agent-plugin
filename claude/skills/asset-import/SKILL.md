...
The published plugin does not carry FFmpeg binaries in Git. The helper resolves explicit `--ffmpeg`/`--ffprobe` arguments and environment overrides first, then a compatible tool on `PATH`. If none is available, it downloads only the pinned asset for the current OS/architecture, verifies declared byte counts plus compressed and extracted SHA-256 hashes, and installs it atomically in the bounded shared ChatCut cache. If the helper still reports a missing or broken media binary, first attempt to fix it yourself using available shell/package-manager capabilities. Only ask the user to install or fix FFmpeg if self-repair is unavailable, blocked, or fails. Do not guess or rewrite helper metadata.
