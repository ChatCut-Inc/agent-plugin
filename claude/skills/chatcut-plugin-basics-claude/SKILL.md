...
Verify the configured server with `claude mcp get plugin:chatcut:chatcut`. If OAuth is needed, launch `sh "${CLAUDE_PLUGIN_ROOT}/skills/chatcut-plugin-basics-claude/login-chatcut.sh"`; it runs the PTY-wrapped login in the background and writes its output to `/tmp/chatcut-login.log`.
...
Do not tell the user only that ChatCut is unavailable, or ask them to "reconnect/enable" it without saying why — that leaves them with no idea what to do. Say that ChatCut needs them to sign in, and give the concrete step: launch `sh "${CLAUDE_PLUGIN_ROOT}/skills/chatcut-plugin-basics-claude/login-chatcut.sh"` and have them complete the browser flow, inspecting `/tmp/chatcut-login.log` if the output is needed. Offer to run it rather than leaving the command sitting there.
...
- Claude Code captures its tool list when a session starts, so the tools usually appear in a **new** session rather than the current one. After a successful sign-in, start a fresh session and check again.
...
For Motion Graphics, load `motion-graphic-gen` and follow the current ChatCut tool schema. Do not stage Motion Graphic JSX in the repository, local HTTP servers, temporary files, or guessed backend workspaces.
