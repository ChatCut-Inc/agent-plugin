...
still the correct route afterward, emit one short localized instruction
followed by exactly one `<clone-voice/>` in the final user-facing assistant
message for that run, then end the message immediately. Do not call another
tool, add more prose, or emit the tag again after it. If another tool call is
still needed, do not emit the tag yet. A single Agent run may render at most
one standalone native clone entry.
...
- In the native ChatCut editor, first check whether the user has already
  supplied a readable audio attachment or an accessible ChatCut audio asset
  specifically for this cloning request. If so, do not make the user choose the
  same file again and do not emit `<clone-voice/>`. Run the creation preflight,
  collect only the missing name, preview text, and explicit authorization, then
  use the Agent-driven cloning flow below with that audio asset.
- In the native ChatCut editor, use the editor dialog only when a usable
  reference has not already been supplied. Give one short localized
...
choice of voice source. A reasonable inference must never bypass the existing
custom-voice question below.
...
- **Clone action selected:** only now enter the host-specific cloning route
  ...
  native editor, first complete any required inspection or preflight, then put
  the single entry in the final assistant message for that run and stop.
- **At least one ready custom voice, but no concrete voice named:** always ask
  which source to use before showing official recommendations—even if the user
  described voice traits or the narration text suggests a tone. This source
  question takes precedence over the branches below. Use `<choices/>` in native
  ChatCut, or the current host adapter's equivalent single-choice surface in
  external hosts.
  - With exactly one ready custom voice, make it the first and specific option,
    for example `Use “Joey”`, followed by `Choose an official voice` and
    `Clone another voice`.
  - With multiple ready custom voices, offer `Choose my cloned voice` first,
    followed by `Choose an official voice` and `Clone another voice`.
  - Preserve any explicit or inferred voice requirements. Apply them only if
    the user chooses the official-voice branch.
- **No ready custom voice and no voice preference:** offer `Official voices`
  and `Clone my voice`, localized to the conversation. This is one branch
  decision, not an audition grid or structured intake form.
...
When traits are explicit, put the action inside the playable grid instead:
...
4. Add one no-media action option with stable value `clone_voice` only when
   this audition is the first voice-selection surface and the user has not
   already chosen the official-voice branch. Label it `Clone my voice` when no
   ready custom voice exists, or `Clone another voice` when one does; localize
   it using the meanings above. In native ChatCut this is a compact
   `<visual-option>` without `media`, not a separate button. In external hosts
   it is the equivalent label-only option. If the user reached this audition by
   choosing the localized official-voice branch defined above, show only
   curated official voices; do not repeat a clone action in the audition grid.
...
preview, keep it selectable by name without inventing media.
...
editor dialog. Render `<clone-voice/>`; do not first ask for the voice name,
language, reference upload, or consent in an Agent widget. The dialog owns
fresh entitlement and slot checks, recording/upload, authorization, durable
source storage, Fish Audio registration, preview, and retry.
...
user chooses the clone branch, proactively send one short, natural sentence
that tells the user they can click the button below to start cloning, followed
by exactly `<clone-voice/>`. Adapt the wording to the conversation and
language. This must be the final assistant message for the run after all
...
that the Agent will collect clone inputs. Never wait for another user prompt or
redirect the user to an editor menu to find cloning themselves.
...
3. Never use explicit or inferred voice traits to skip the source-choice prompt
   when at least one ready custom voice exists.
4. Never repeat a clone action in the official audition after the user already
   chose the official-voice branch.
...
14. Never emit `<clone-voice/>` in an intermediate message or more than once in
    ...
    the correct route, emit one final localized instruction plus one exact tag,
