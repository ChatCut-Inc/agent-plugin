...
const TRANSCRIPTION_WAVEFORM_CONCURRENCY = 1;
...
const HDR_TRANSFERS = new Set(["arib-std-b67", "hlg", "pq", "smpte2084"]);
const SDR_BT709_COLOR_ARGS = [
  "-color_primaries",
  "bt709",
  "-color_trc",
  "bt709",
...
Get the token once from import_media, then pass all readable media files to
this script, up to ${MAX_INPUT_FILES} at a time. Files run in parallel. Non-SVG assets are
...
function parseArgs(argv) {
  const options = {
    ...
    workDir: process.env.CHATCUT_MEDIA_WORK_DIR || "",
  };
...
async function probeMedia(path, options) {
  const { stdout } = await run(
    ...
    [
      ...
      "error",
      "-show_entries",
      "format=duration,bit_rate:stream=index,codec_type,codec_name,width,height,pix_fmt,color_range,color_space,color_transfer,color_primaries,bits_per_raw_sample,avg_frame_rate,r_frame_rate,bit_rate,start_time,duration,channels,sample_rate:stream_tags=rotate:stream_side_data=rotation",
      "-of",
      "json",
      path,
    ],
  ...
  return {
    ...
    bitsPerRawSample: numberOrUndefined(video?.bits_per_raw_sample),
    size,
    sourceBitrate,
...
export function isHdrVideo(metadata) {
  return HDR_TRANSFERS.has(String(metadata.colorTransfer || "").toLowerCase());
}

function zscaleInputValue(value, fallback) {
  const normalized = String(value || "").toLowerCase();
  if (normalized === "hlg") return "arib-std-b67";
  if (normalized === "pq") return "smpte2084";
  if (
    !normalized ||
    ["unknown", "unspecified", "reserved"].includes(normalized)
  )
    return fallback;
  return normalized;
}

export function videoFilter(metadata, targetWidth, targetHeight) {
  const scale = `scale=${targetWidth}:${targetHeight}:flags=lanczos`;
  if (!isHdrVideo(metadata)) return scale;

  // Tone-map before reducing to 8-bit so HLG/PQ is never mislabeled as SDR.
  const transfer = zscaleInputValue(metadata.colorTransfer, "arib-std-b67");
  const primaries = zscaleInputValue(metadata.colorPrimaries, "bt2020");
  const matrix = zscaleInputValue(metadata.colorSpace, "bt2020nc");
  const range = ["full", "jpeg", "pc"].includes(
    String(metadata.colorRange || "").toLowerCase(),
  )
    ? "full"
    : "limited";
  return [
    `zscale=primariesin=${primaries}:transferin=${transfer}:matrixin=${matrix}:rangein=${range}:transfer=linear:npl=100`,
    "format=gbrpf32le",
...
export function outputColorArgs(metadata) {
  return isHdrVideo(metadata) ? SDR_BT709_COLOR_ARGS : [];
}

export function videoTranscodeReason(metadata, targetBitrate) {
  if (isHdrVideo(metadata)) {
    return `HDR transfer ${metadata.colorTransfer} requires SDR BT.709 tone mapping`;
  }
...
function vaapiArgs(
  ...
  const vf = isHdrVideo(metadata)
    ? `${videoFilter(metadata, targetWidth, targetHeight)},format=nv12,hwupload`
...
async function transcodeVideo(input, metadata, context) {
  ...
  const vf = videoFilter(metadata, targetWidth, targetHeight);
  const mp4Path = join(
    context.workDir,
  ...
  for (const encoder of await hardwareH264Encoders(context.options)) {
    ...
    args.push(...outputColorArgs(metadata), "-movflags", "+faststart");
...
async function prepareTranscriptionWaveform(audio, durationSeconds, context) {
  ...
  return withWaveformSlot(async () => {
    const peaks = await streamWaveformPeaks(audio.path, expected, context);
    if (!peaks) return undefined;
    const outputPath = join(
      context.workDir,
      `chatcut-${context.nameNoExt}-waveform-${process.pid}-${context.index}.u8`,
    );
    await writeFile(outputPath, peaks);
    return {
...
async function withWaveformSlot(task) {
  if (TRANSCRIPTION_WAVEFORM_CONCURRENCY !== 1) return task();
  const previous = waveformQueue;
  let release;
  waveformQueue = new Promise((resolve) => {
    release = resolve;
  });
  await previous;
  try {
...
async function streamWaveformPeaks(input, expected, context) {
  return new Promise((resolve) => {
    const child = spawn(
      ...
      [
        ...
        input,
        "-af",
        "aresample=48000,asetnsamples=n=480:p=0,astats=metadata=1:reset=1:measure_perchannel=none:measure_overall=Peak_level,ametadata=print:key=lavfi.astats.Overall.Peak_level:file='pipe\\:1'",
        "-f",
        "null",
        "-",
      ],
    ...
    const peaks = new Uint8Array(expected);
    let buffered = "";
    let count = 0;
    let overflow = false;
    ...
    const consume = (line) => {
      ...
      peaks[count] = Number.isFinite(db)
        ? Math.round(Math.min(1, 10 ** (db / 20)) * 255)
        : 0;
      count += 1;
    };
    ...
    child.on("close", (code) => {
      ...
      resolve(code === 0 && !overflow && count === expected ? peaks : undefined);
    });
  });
}
...
function metadataJson(
  ...
  return {
    ...
    ...(upload.path === sourcePath ? { uploadPath: upload.path } : {}),
...
async function runSingleImport(index, total, sourceFile, options) {
  ...
  if (!isResumeUpload) {
    ...
    const requestedAssetId = registeredAssetId;
    registeredAssetId = registerResult.assetId || requestedAssetId;
    partialItem = {
      sourcePath: sourceFile,
...
async function runSessionImport(options) {
  ...
  const finalItems = new Array(options.inputs.length);
  await Promise.all(
    registrations.map(async (registration, index) => {
      const sourcePath = options.inputs[index];
      if (registration.status === "rejected") {
        finalItems[index] = failedImport(registration.reason, sourcePath);
        return;
      }

      const entry = registration.value;
      if (!entry.deferredUpload) {
        finalItems[index] = entry;
        return;
      }

      try {
        finalItems[index] = await entry.deferredUpload();
      } catch (error) {
        finalItems[index] = failedImport(
          error,
          sourcePath,
          entry.partialItem,
        );
  ...
  const failures = finalItems.filter((item) => item?.ok === false);
  if (failures.length > 0) {
    const error = new Error(
      `${failures.length} of ${options.inputs.length} media imports failed.`,
    );
    error.imports = finalItems;
    throw error;
  }
...
function failedImport(error, sourcePath, partialItem = {}) {
  const failure = error instanceof Error ? error : new Error(String(error));
  partialItem = failure.partialItem || partialItem;
  return {
    ...partialItem,
    ok: false,
    sourcePath: failure.sourcePath || partialItem.sourcePath || sourcePath,
...
function registeredAssetFailure(
  error,
  assetId,
  sourcePath,
  options,
  ...
  const failure = error instanceof Error ? error : new Error(String(error));
  failure.assetId = assetId;
  failure.sourcePath = sourcePath;
  failure.retry = retryPlan(assetId, sourcePath, options);
  failure.partialItem = partialItem;
  return failure;
}
...
async function main() {
  ...
  if (!options.sessionToken) {
    usage();
    fail(
      "--token is required. Call import_media to create an upload session first.",
    );
  }
  if (!options.endpoint) {
    usage();
  ...
  let ownedWorkDir = "";
  if (!options.workDir) {
    ownedWorkDir = await mkdtemp(join(tmpdir(), "chatcut-media-import-"));
    options.workDir = ownedWorkDir;
  }
  try {
    ...
    if (ownedWorkDir) await rm(ownedWorkDir, { recursive: true, force: true });
  }
}
      ...
      `${JSON.stringify(
        {
          ...
          ...(error.imports ? { imports: error.imports } : {}),
        },
        null,
        2,
      )}\n`,
