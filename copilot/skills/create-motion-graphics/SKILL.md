...
- Load `widget-forms` and request one structured visual-direction choice with
  the exact preset ids. Do not emit raw ChatCut widget tags.
...
`action: "list_presets"`; pass `scenario` only when it is clear. Pick 3-6
matches using each preset's `description` as agent-facing matching guidance.
Load `widget-forms` and put each exact returned preset id in the
`playable_single_choice` value map with its current name as the label. Copilot
CLI does not render ChatCut's raw `<visual-option>` tags, so never emit them.
If safe preview images are available through a host-native surface, show them
before the label-based form; otherwise say that the presets are visual starting
points and the user can pick a close option or describe a different direction.
Do not mix custom non-preset directions into the same choice list.
...
current style-picking exchange; show fewer than 6 rather than repeat. Apply the
user's pick with `action: "apply_preset"`. After applying a preset or saved
style, call `manage_design_style` with `action: "get"` before authoring MG code;
preset names and `list_presets` descriptions are picker guidance, not the full
motion/design spec. Do not create or update a Design Style from an unconfirmed
recommendation.
