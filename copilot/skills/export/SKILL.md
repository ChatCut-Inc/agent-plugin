...
When a render completes, return its `downloadUrl`. In GitHub Copilot CLI,
download the completed file to a collision-safe local path and provide the path,
local-file preview link, and concise render metadata. Do not claim delivery
from a queued or running render.
