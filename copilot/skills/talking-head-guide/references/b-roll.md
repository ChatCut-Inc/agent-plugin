...
Footage can come from three places: clips already in the project library,
stock via `search_stock_media` followed by downloading the selected result and
importing it through `asset-import` and `import_media`, or AI generation via
the `video-gen` skill (Seedance 2.0, ChatCut Pro only). Pick based on the
user's need; if unclear, align with the user upfront.
