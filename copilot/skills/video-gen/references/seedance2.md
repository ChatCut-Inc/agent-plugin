...
All frame/reference slots take a project **asset id** (UUID or short prefix
from `browse_assets`), `asset://<id>`, or a same-project asset URL. External
URLs are not accepted — download them into the workspace, then import the
local files through `asset-import` and `import_media`.
...
- **Generated character / illustration** → use a host-provided image-generation
  capability when available, save the result as a local image, import it
  through `asset-import`, then pass its asset id in `refImages`. If no such
  capability is available, ask the user to provide or upload the image.
...
- **Generated photorealistic character** → do not promise that an arbitrary
  generated image will be trusted. Upstream trust is limited to eligible,
