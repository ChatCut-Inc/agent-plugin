...
can install and edit, but shows editor links instead of an embedded browser.
...
e.g. `preview_start {url}`), keeping `dockviewLayout` and the one-time
`editor-boot-token` (it signs the editor in automatically) but removing
`theme=codex` — that parameter enables a Codex-webview mode that blocks
manual file drops and hides import buttons, which must stay usable in the
Claude Code pane. The
