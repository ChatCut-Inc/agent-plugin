...
description: "Use whenever Grok Bot needs to add, create, hand-author, patch, or place Motion Graphic JSX assets in a ChatCut cloud project. Covers direct inline JSX authoring, visual language, editable properties, asset binding, timeline placement, and verification. Not for the built-in ChatCut Agent and not for ChatCut Desktop."
---

# Create Motion Graphics

Use this skill when Grok Bot requires a Motion Graphic asset authored or patched as inline JSX on a ChatCut cloud project. The built-in ChatCut Agent uses `motion-graphic-gen` and `submit_motion_graphic` instead. ChatCut Desktop is not available on this host.
...
Pass Motion Graphic source inline through the hosted ChatCut MCP tool (`create_motion_graphic_from_code` / `edit_asset`). Do not stage code in the ChatCut repository, `ai-working/`, `/tmp`, a local HTTP server, generated code files, or guessed application paths when the tool accepts inline JSX directly. Never emit ChatCut `<widget>` tags; load `widget-forms` for style pickers.
