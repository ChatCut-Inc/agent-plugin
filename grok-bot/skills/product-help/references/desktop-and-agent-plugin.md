...
The ChatCut Agent Plugin connects ChatGPT/Codex, Claude Code, Grok Build, or Grok Bot to ChatCut projects. Prefer the in-product installation flow:

1. Click the profile icon in the top-right corner.
2. Open **Agent Plugin**.
3. Click **Copy** beside **ChatGPT/Codex**, **Claude Code**, **Grok Build**, or **Grok Bot** when that row exists.
4. On the computer where the user wants to edit, open the matching host, paste the copied installation prompt into a session, then send it.
...
If the Grok Build Copy row is not in the menu yet, paste this prompt into Grok Build:

```text
Read https://github.com/ChatCut-Inc/agent-plugin/blob/main/docs/grok-build-install.md to install the ChatCut plugin and set up a new ChatCut task for me.
```
...
- Grok Build: repository file `docs/grok-build-install.md` (site route `https://chatcut.io/grok` when wired)
- Grok Bot: repository file `docs/grok-bot-install.md` (site route `https://chatcut.io/grok-bot` when wired)

Grok Build has no in-app editor pane. After install, open the returned ChatCut `editorUrl` in the system browser, or connect ChatCut Desktop with `grok mcp add chatcut_desktop -- "$HOME/Library/Application Support/ChatCut/chatcut-mcp"`.
