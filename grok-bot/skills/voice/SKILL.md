...
3. After the tag, add one short, localized sentence asking the user to send a
   message when cloning is complete so the Agent can continue the request.

Adapt the wording to the conversation and language. This combined entry and
reminder must be the final assistant message for the run after all required
tools have finished; never place it in a pre-tool or intermediate message.
Examples:
