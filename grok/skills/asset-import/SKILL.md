...
Use this host adapter for files the current plugin host can read locally. The hosted external MCP does not expose the backend agent's `push_asset` tool, so do not follow a shared `push_asset` workflow in this plugin.
...
Resolve the helper relative to this skill; do not search the workspace. Use the host's bundled Node runtime when available, otherwise Node 18 or newer from `PATH`. Run the helper in the foreground and read its final JSON from stdout. Do not detach it or invent status files.
