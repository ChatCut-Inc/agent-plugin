...
If `manage_avatar` and `submit_avatar_video` are not already visible, discover them with the host tool catalog (`search_tool` on Grok Build, or the equivalent tool search on this host). Call `manage_avatar` for live capabilities before offering creation or advanced options.
