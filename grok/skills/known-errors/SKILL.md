...
- Hosted `chatcut` with an empty tool list is almost always missing OAuth, not a broken plugin. In Grok Build open `/mcps`, select `chatcut`, press `i`. There is no `grok mcp login` command. Start a new session after sign-in.
- `chatcut_desktop` missing is a Desktop connection issue. Load `connect-chatcut-desktop`. Do not reinstall the hosted plugin to fix Desktop.
- Sign-in is per ChatCut environment. Production `api.chatcut.io` is not a local dev stack.
...
- If media conversion fails because ffmpeg is missing, install system ffmpeg or ask the user to upload in the ChatCut editor. This Grok plugin does not vendor ffmpeg binaries.
...
- Use only generation tools visible on the current host. Direct-author Motion Graphics with `create_motion_graphic_from_code` when that tool exists.
