...
The source package is version **0.1.5**. It follows the independent-directory delivery used by `plugins/grok` (PR #4099), but draws craft from the current repository rather than copying Grok's older MG instructions. Source baseline: `600fa53770`.
...
```sh
cd plugins/kimi
zip -r /path/to/chatcut-kimi-work-0.1.5.zip .
```
