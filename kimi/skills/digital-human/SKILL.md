...
The raw tags below describe the embedded ChatCut protocol. In Kimi Work, use the host's supported preview/choice mechanics. Map each identity's `identityId`, `name`, `previewVideoUrl`, and `previewImageUrl` to that surface when available. Keep `create_avatar` as a separate choice returned to this workflow.
...
- `create_avatar` — opens the native creation dialog where the user uploads a photo
  ...
  avatar" phrase (e.g. Create your own avatar); do not mention uploading a photo — the
...
```text
<widget>
  <form-visual id="avatar" question="Choose an avatar" required="true">
...
```text
<widget>
  <form-files id="avatar_source" label="Upload an avatar photo"
...
```text
<widget>
  <form-visual id="voiceRef" label="Choose and preview a voice" media-kind="audio" required="true">
  ...
  <form-textarea id="script" label="Confirm the script" required="true" default="<script>"/>
</widget>
```
