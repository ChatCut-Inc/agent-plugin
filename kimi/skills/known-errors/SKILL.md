...
- Follow `chatcut-media-guide` for the Kimi host's supported import path; do not substitute backend-only `push_asset` or `download_media` calls when they are absent.
...
- Use only generation tools visible on the current host. External plugins direct-author Motion Graphics with `create_motion_graphic_from_code` when available; the built-in ChatCut Agent may expose `submit_motion_graphic`.
...
- A successful mutation is not visual proof; load `chatcut-verification-guide` when the result must be seen.
- Use `track_export` for render jobs. If cloud render cannot read an asset, follow `chatcut-media-guide` only with user permission, or report the limitation.
