...
All frame/reference slots take a project **asset id** (UUID or short prefix from `browse_assets`), `asset://<id>`, or a same-project asset URL. External URLs are not accepted — follow `chatcut-media-guide` to import supported files first.
