...
5. **Render the editable source-text review for lip-sync only.** Use the host's
   supported editable-text question and reuse the same confirmation fields
   ...
   - In an embedded raw-tag route, apply the host's XML attribute
   ...
   ```text
   Please review the recognized source transcript and correct any errors:

   <widget>
     <form-textarea id="reviewedSourceTranscript" label="Confirm and correct the source transcript" rows="12" required="true" default="<recognized source text; one segment per line>"/>
   </widget>
   ```

   Follow the active host's interaction mechanics rather than emitting raw tags
   where the embedded protocol is unsupported. Do not add a separate
