...
Decide the host path before asking for clone inputs:
...
- In external plugin hosts, never emit `<clone-voice/>`; those hosts do
...
audio with ffmpeg when the host supports it, then load `chatcut-media-guide` and import the result through the Kimi-supported flow. For any
...
- **Clone action selected:** only now enter the host-specific cloning route
  ...
  dialog entry; in external plugin hosts, begin the attachment flow.
...
Use the current host's supported form, attachment, or media-card behavior before
collecting input or rendering a choice. This skill
...
3. Request one required playable single choice through the current host. Give
...
- an external plugin host is collecting the reference as a conversation
...
3. Request only the missing parts of this host-neutral
   intake contract:
...
readable attachment instead, load `chatcut-media-guide`, import it into the targeted
...
ready voice and `previewUrl`, use the current host to render
...
13. Never render a cloned-voice preview as raw `<audio>` HTML, a Markdown link,
    or a bare URL. Use the host's playable preview surface with the exact
